...
class PostController extends AbstractController
{
    ...
    public function show(Post $post, Request $request, CommentHandler $commentHandler): Response
    {
        ...
        $commentsCount = count($this->commentRepository->findBy(
            ['post' => $post],
            ['created_at' => 'DESC']
        ));
    ...
    public function loadMoreComment(Post $post, Request $request): Response
    {
        $comments = $this->paginator->paginate(
            $this->commentRepository->findBy(['post' => $post], ['created_at' => 'DESC']),
